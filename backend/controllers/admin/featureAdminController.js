...
const defaultFeatures = [
  ...
  { key: "registrations_paused", label: "Pause New Registrations", description: "When enabled, blocks all new patient/doctor account registrations platform-wide", isEnabled: false },
];
...
export const updateFeatureToggle = asyncHandler(async (req, res) => {
  ...
  // Phase A6.2.3 — Automation Studio real trigger. Note:
  // actionExecutor.js's own feature_toggle action writes to FeatureToggle
  // directly (bypassing this controller), so a flow that toggles a feature
  // can never re-trigger itself into a loop here.
  await emitAutomationTrigger(TRIGGER_TYPES.FEATURE_TOGGLE_CHANGED, {
    key,
