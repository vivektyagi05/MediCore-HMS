...
const { EMPTY_LOCATION, OTHER_OPTION, changeLevel, setCityText, setOtherText, locationFromDoctor, buildLocationPayload, selectValueFor, isDistrictChosen } = u;
...
test("typing a city drops any matched canonical id (server re-resolves)", () => {
  const next = setCityText(full, "Sanganer");
  assert.equal(next.cityMasterId, ""); assert.equal(next.cityType, "OTHER"); assert.equal(next.cityOther, "Sanganer"); assert.equal(next.city, "Sanganer");
  assert.equal(setCityText(full, "").cityType, "");
});
...
test("stored canonical ids/types are restored after reload (persistence)", () => {
  ...
  assert.equal(loc.stateMasterId, "S1"); assert.equal(loc.districtMasterId, "D1"); assert.equal(loc.city, "Jaipur"); assert.equal(loc.cityType, "MASTER");
  assert.equal(selectValueFor(loc, "state"), "S1");
});
test("stored OTHER city is shown as text", () => {
  const loc = locationFromDoctor({ stateMasterId: "S1", stateType: "MASTER", state: "R", districtMasterId: "D1", districtType: "MASTER", district: "J", cityType: "OTHER", city: "Some Colony", cityOther: "Some Colony" });
  assert.equal(loc.city, "Some Colony"); assert.equal(loc.cityType, "OTHER");
});
test("payload: canonical state+district with typed city", () => {
  const p = buildLocationPayload(setCityText(changeLevel(changeLevel(EMPTY_LOCATION, "state", { id: "S1", name: "Rajasthan" }), "district", { id: "D1", name: "Jaipur" }), "Jaipur"));
  assert.equal(p.stateType, "MASTER"); assert.equal(p.stateMasterId, "S1"); assert.equal(p.districtMasterId, "D1");
  assert.equal(p.city, "Jaipur"); assert.equal(p.cityType, "OTHER"); assert.equal(p.cityMasterId, "");
});
...
test("payload: city text without a district is never sent", () => {
  const p = buildLocationPayload({ ...EMPTY_LOCATION, city: "Orphan", cityType: "OTHER", cityOther: "Orphan" });
  assert.equal(p.city, "");
});
test("payload: OTHER state + OTHER district + typed city", () => {
  const p = buildLocationPayload({ ...EMPTY_LOCATION, stateType: "OTHER", stateOther: "Abroad", districtType: "OTHER", districtOther: "Somewhere", city: "Town" });
  assert.equal(p.stateType, "OTHER"); assert.equal(p.stateOther, "Abroad"); assert.equal(p.districtOther, "Somewhere"); assert.equal(p.cityOther, "Town");
});
