...
import { buildAISystemHealth } from "../services/ai/aiSystemHealthAggregates.js";
...
// PHASE UI-12 — AI System Health. See backend/services/ai/
// aiSystemHealthAggregates.js for the full audit note: reuses the existing
// provider-status check and the real AIDraft generation log, never invents
// an uptime percentage or an error rate.
export const getAISystemHealth = asyncHandler(async (_req, res) => {
  const data = await buildAISystemHealth();
  res.status(200).json({ success: true, data, message: "AI system health fetched successfully" });
});
