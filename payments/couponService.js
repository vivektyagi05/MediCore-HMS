...
export const couponService = {
  ...
  async markUsed({ coupon, userId, paymentId }) {
    if (!coupon) return;
    // Idempotency guard: this can be re-invoked by payment post-processing
    // retries/reconciliation. Without this check a retry would increment
    // usedCount twice and push a duplicate usedBy entry for the same payment.
