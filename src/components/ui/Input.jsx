function Input({ label, error, helperText, required, className = "", id, ...props }) {
  ...
  return (
    <label htmlFor={inputId} className="block">
      ...
      />
      {error ? (
        <span id={`${inputId}-error`} className="mt-1.5 block text-xs font-medium text-rose-600">{error}</span>
      ) : helperText ? (
        <span id={`${inputId}-helper`} className="mt-1.5 block text-xs font-medium text-slate-500">{helperText}</span>
      ) : null}
    </label>
  );
}
